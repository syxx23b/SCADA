...
$installerOutputRoot = $repoRoot
$techStackNodeModules = 'C:\Users\syxxz\OneDrive\SCADA\Tech Stack\Scada_Codex\node_modules'
...
function Remove-PathIfExists {
    ...
    if (Test-Path -LiteralPath $LiteralPath) {
        try {
            ...
            Write-Warning "Cleanup skipped for '$LiteralPath': $($_.Exception.Message)"
        }
    }
}
...
function Ensure-NodeModulesLink {
    ...
    if (Test-Path -LiteralPath $LinkPath) {
        ...
        # OneDrive may leave an online-only placeholder at an ignored build path.
        # Replace that placeholder so Node resolves packages from the shared cache.
...
function Clear-BuildOutputs {
    $paths = @(
        ...
        (Join-Path $repoRoot 'Scada.Api\scada-api-9001.log'),
        $tempPublishRoot,
