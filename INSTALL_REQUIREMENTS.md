...
- The API listens on port `9001` by default.
- Local deployment and verification target `http://localhost:9001`.
...
- The default open port is TCP `9001`.
...
- Use backend-served `http://localhost:9001` mainly for integrated verification of API proxying, SignalR, static publish output, and final deployment behavior.
...
- The final installer output directory is the workspace root (the directory that contains `Scada.slnx`), e.g. `SCADA-Setup-2026.09.06.1700.exe` directly next to `Scada.slnx`.
...
- Cleanup should remove at least:
  ...
  - `scada-web/dist`
  - transient runtime log files such as `Scada.Api/scada-api-9001.log`
