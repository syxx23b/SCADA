...
builder.Services.AddHostedService<RealTimeDataRecorderHostedService>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    EnsureRealTimeDataSchema(dbContext);
    EnsureLocalDeviceAndTags(dbContext);
    EnsureSchemaMigrationsTable(dbContext);
    RunOneTimeMigration(dbContext, "legacy-products-cleanup", """
...
{
    // EfficiencyTimelineSegments is created idempotently inside EnsureScadaCoreTables.
}
...
static void EnsureRealTimeDataSchema(ScadaDbContext dbContext)
{
    const string ensureRealTimeDataSql = """
IF OBJECT_ID(N'[dbo].[RealTimeData]', N'U') IS NULL
BEGIN
    CREATE TABLE [dbo].[RealTimeData](
        [ID] bigint IDENTITY(1,1) NOT NULL CONSTRAINT [PK_RealTimeData] PRIMARY KEY,
        [sj] datetime NULL,
        [gw] int NULL,
        ...
        [flow] float NULL,
        [siphon] float NULL,
        [inletTemp] float NULL,
        [speed] float NULL
    );
END
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_RealTimeData_sj' AND object_id = OBJECT_ID(N'[dbo].[RealTimeData]'))
BEGIN
    CREATE INDEX [IX_RealTimeData_sj] ON [dbo].[RealTimeData]([sj]);
END
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_RealTimeData_gw_sj' AND object_id = OBJECT_ID(N'[dbo].[RealTimeData]'))
BEGIN
    CREATE INDEX [IX_RealTimeData_gw_sj] ON [dbo].[RealTimeData]([gw], [sj]);
END
""";
    dbContext.Database.ExecuteSqlRaw(ensureRealTimeDataSql);
}
...
static void EnsureSchemaMigrationsTable(ScadaDbContext dbContext)
{
    const string ensureSchemaMigrationsSql = """
IF OBJECT_ID(N'[Process].[SchemaMigrations]', N'U') IS NULL
BEGIN
    CREATE TABLE [Process].[SchemaMigrations](
        [Name] nvarchar(128) NOT NULL CONSTRAINT [PK_SchemaMigrations] PRIMARY KEY,
        [AppliedAt] datetimeoffset(7) NOT NULL
    );
END
""";
    dbContext.Database.ExecuteSqlRaw(ensureSchemaMigrationsSql);
}

static void RunOneTimeMigration(ScadaDbContext dbContext, string name, string migrationSql)
{
    // name/migrationSql are compile-time constants defined at the call sites below,
    // so plain concatenation is safe here (avoids EF1002 on interpolated ExecuteSqlRaw).
    var runOneTimeMigrationSql =
        "IF NOT EXISTS (SELECT 1 FROM [Process].[SchemaMigrations] WHERE [Name] = N'" + name + "')\n" +
        "BEGIN\n" +
        migrationSql + "\n" +
        "\n" +
...
{
    const string ensureRepairRecordSql = """
...
END;
""";
    dbContext.Database.ExecuteSqlRaw(ensureRepairRecordSql);
}
