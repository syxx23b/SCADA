...
public sealed class SystemSettingsController : ControllerBase
{
    ...
    private static readonly string[] DefaultVisibleMenuKeys =
    [
    ...
    private static readonly string[] RequiredVisibleMenuKeys =
    [
        "workOrderCreate",
    ];
    ...
    public async Task<ActionResult<SystemSettingsResponseDto>> Update([FromBody] UpdateSystemSettingsRequestDto request, CancellationToken cancellationToken)
    {
        ...
        UpsertSetting(settings, "VisibleMenuKeys", string.Join(',', NormalizeVisibleMenuKeys(request.VisibleMenuKeys)), now);
    ...
    private async Task<Dictionary<string, SystemSettingEntity>> EnsureSettingsAsync(CancellationToken cancellationToken)
    {
        ...
        changed |= EnsureDefault(settings, "VisibleMenuKeys", string.Join(',', DefaultVisibleMenuKeys), now);
        changed |= EnsureRequiredVisibleMenuKeys(settings, now);
    ...
    private void UpsertSetting(Dictionary<string, SystemSettingEntity> settings, string key, string value, DateTimeOffset now)
    {
        ...
        var newEntity = new SystemSettingEntity
        {
            Key = key,
            Value = value,
            UpdatedAt = now,
        };
        _dbContext.SystemSettings.Add(newEntity);
        settings[key] = newEntity;
    }

    private bool EnsureRequiredVisibleMenuKeys(Dictionary<string, SystemSettingEntity> settings, DateTimeOffset now)
    {
        if (!settings.TryGetValue("VisibleMenuKeys", out var entity))
        {
            return false;
        }

        var normalized = NormalizeVisibleMenuKeys(entity.Value.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries));
        if (RequiredVisibleMenuKeys.All(required => normalized.Contains(required, StringComparer.OrdinalIgnoreCase)))
        {
            return false;
        }

        entity.Value = string.Join(',', NormalizeVisibleMenuKeys(normalized.Concat(RequiredVisibleMenuKeys)));
        entity.UpdatedAt = now;
        return true;
    }
    ...
    private static SystemSettingsResponseDto ToResponse(Dictionary<string, SystemSettingEntity> settings)
    {
        ...
        var visibleMenuKeys = NormalizeVisibleMenuKeys(ParseString(settings, "VisibleMenuKeys", string.Join(',', DefaultVisibleMenuKeys)).Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries));
        return new SystemSettingsResponseDto(stationCount, pressureUnit, flowUnit, visibleMenuKeys);
    }
    ...
    private static string[] NormalizeVisibleMenuKeys(IEnumerable<string>? values)
    {
        if (values is null)
        {
            return DefaultVisibleMenuKeys;
        }

        var allowed = DefaultVisibleMenuKeys.ToHashSet(StringComparer.OrdinalIgnoreCase);
        return values
