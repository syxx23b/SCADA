...
public sealed class ProductionController : ControllerBase
{
    ...
    private static readonly FactoryTestReportColumnDto[] MotorFaultReportColumns =
    [
        new("sj", "时间", "排产信息", "center"),
        new("mode", "模式", "排产信息", "center"),
        new("tm", "条码", "排产信息", "left"),
        new("gw", "工位", "排产信息", "right"),
        new("model", "型号", "排产信息", "left"),
        new("ERRinformation", "故障描述", "故障信息", "left"),
        new("current", "电流", "测试数据", "right"),
        new("pressure", "压力", "测试数据", "right"),
        new("flow", "流量", "测试数据", "right"),
        new("speed", "转速", "测试数据", "right"),
    ];

    private const string MotorFaultReportCountSql = """
        SELECT COUNT_BIG(1)
        FROM dbo.[Error] AS E
        WHERE E.sj >= @from
          AND E.sj <= @to
          AND E.gw > 0
          AND (@gw IS NULL OR E.gw = @gw)
          AND (@orderNo IS NULL OR LTRIM(RTRIM(CONVERT(nvarchar(100), E.[orderNo]))) = @orderNo)
          AND (@tm IS NULL OR LTRIM(RTRIM(CONVERT(nvarchar(100), E.tm))) = @tm)
          AND (@err IS NULL OR E.[ERR] = @err);
        """;

    private const string MotorFaultReportRowsSql = """
        SELECT TOP (@maxRows)
            E.sj,
            E.mode,
            E.tmJS,
            ...
            E.model,
            E.[ERR],
            COALESCE(NULLIF(LTRIM(RTRIM(CONVERT(nvarchar(200), E.ERRinformation))), ''), D.ERRinformation) AS ERRinformation,
            E.[current],
            E.pressure,
            E.flow,
            E.speed
    ...
    public async Task<ActionResult<FaultByGwResponseDto>> GetTodayFaultByGw(CancellationToken cancellationToken)
    {
        ...
        const string quarterErrorSql = """
            ...
            WHERE gw > 0
              AND [ERR] > 0
              AND sj >= DATEADD(month, -12, CAST(GETDATE() AS date))
              AND sj < DATEADD(day, 1, CAST(GETDATE() AS date))
    ...
    [HttpGet("motor-fault-report")]
    public async Task<ActionResult<FaultTrackingReportResponseDto>> GetMotorFaultReport(
        [FromQuery] DateTime from,
        [FromQuery] DateTime to,
        [FromQuery] int? gw,
        [FromQuery] int? err,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            return Ok(await QueryMotorFaultReportAsync(from, to, gw, err, orderNo, tm, cancellationToken));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to query motor fault report from MSSQL");
            return StatusCode(StatusCodes.Status500InternalServerError, new { message = "电机泵故障追踪报表查询失败" });
        }
    }

    [HttpGet("gas-engine-fault-report")]
    public async Task<ActionResult<FaultTrackingReportResponseDto>> GetGasEngineFaultReport(
        [FromQuery] DateTime from,
        [FromQuery] DateTime to,
        [FromQuery] int? gw,
        [FromQuery] int? err,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex, "Failed to query gas engine fault report from MSSQL");
            return StatusCode(StatusCodes.Status500InternalServerError, new { message = "汽油机故障追溯报表查询失败" });
        }
    }

    [HttpGet("motor-fault-report/trace")]
    public async Task<ActionResult<FaultTraceResponseDto>> GetMotorFaultTrace(
        [FromQuery] DateTime occurredAt,
        [FromQuery] int gw,
        CancellationToken cancellationToken)
    {
        if (gw <= 0)
        {
            return BadRequest(new { message = "工位必须大于 0" });
        }

        const string sql = """
            SELECT sj, [current], pressure, flow, speed
            FROM dbo.RealTimeData
            WHERE gw = @gw
              AND sj >= DATEADD(minute, -2, @occurredAt)
              AND sj <= DATEADD(second, 30, @occurredAt)
            ORDER BY sj ASC, ID ASC;
            """;

        try
        {
            var points = new List<FaultTracePointDto>();
            await using var connection = new SqlConnection(_connectionString);
            await connection.OpenAsync(cancellationToken);
            await using var command = new SqlCommand(sql, connection);
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                points.Add(new FaultTracePointDto(
                    reader.GetDateTime(0).ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture),
                    reader.IsDBNull(1) ? null : Convert.ToDecimal(reader.GetValue(1), CultureInfo.InvariantCulture),
            ...
            return Ok(new FaultTraceResponseDto(
                occurredAt.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture),
                gw,
                occurredAt.AddMinutes(-2).ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture),
                occurredAt.AddSeconds(30).ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture),
                points));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to query motor fault trace from RealTimeData");
            return StatusCode(StatusCodes.Status500InternalServerError, new { message = "故障追溯数据查询失败" });
        }
    }

    [HttpGet("gas-engine-fault-report/trace")]
    public Task<ActionResult<FaultTraceResponseDto>> GetGasEngineFaultTrace(
        [FromQuery] DateTime occurredAt,
        [FromQuery] int gw,
        CancellationToken cancellationToken)
        => GetMotorFaultTrace(occurredAt, gw, cancellationToken);
    ...
    private async Task<FactoryTestReportUnits> GetFactoryTestReportUnitsAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            return new FactoryTestReportUnits(
                ...
                NormalizeFactoryTestReportUnit(settings.GetValueOrDefault("FlowUnit"), ["L/M", "m³/h", "GPM"], "L/M"));
        }
        catch (Exception ex)
        {
    ...
    private async Task<FaultTrackingReportResponseDto> QueryMotorFaultReportAsync(
        DateTime from,
        DateTime to,
        int? gw,
        int? err,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var normalizedTm = string.IsNullOrWhiteSpace(tm) ? null : tm.Trim();

        await using var connection = new SqlConnection(_connectionString);
        await connection.OpenAsync(cancellationToken);

        await using (var countCommand = new SqlCommand(MotorFaultReportCountSql, connection))
        {
            AddMotorFaultReportParameters(countCommand, from, to, gw, err, normalizedOrderNo, normalizedTm);
            var totalCount = Convert.ToInt32(await countCommand.ExecuteScalarAsync(cancellationToken) ?? 0L);

            await using var rowsCommand = new SqlCommand(MotorFaultReportRowsSql, connection);
            AddMotorFaultReportParameters(rowsCommand, from, to, gw, err, normalizedOrderNo, normalizedTm);
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                var row = new Dictionary<string, string?>(MotorFaultReportColumns.Length, StringComparer.OrdinalIgnoreCase);
                foreach (var column in MotorFaultReportColumns)
                {
                    var ordinal = reader.GetOrdinal(column.Key);
                    row[column.Key] = reader.IsDBNull(ordinal)
                        ? null
                        : reader.GetValue(ordinal) switch
                        {
                            ...
                            _ => Convert.ToString(reader.GetValue(ordinal), CultureInfo.InvariantCulture)
                        };
                }
                rows.Add(row);
            }

            return new FaultTrackingReportResponseDto(
                ...
                MotorFaultReportColumns,
                rows,
                DateTimeOffset.Now.ToString("O", CultureInfo.InvariantCulture));
        }
    }

    private static void AddMotorFaultReportParameters(
        SqlCommand command,
        DateTime from,
        DateTime to,
        int? gw,
        ...
        string? tm)
    {
        ...
        command.Parameters.AddWithValue("@gw", gw.HasValue ? gw.Value : DBNull.Value);
        command.Parameters.AddWithValue("@err", err.HasValue ? err.Value : DBNull.Value);
        command.Parameters.AddWithValue("@orderNo", orderNo is null ? DBNull.Value : orderNo);
        command.Parameters.AddWithValue("@tm", tm is null ? DBNull.Value : tm);
    }
...
public sealed record FaultTrackingReportResponseDto(
    string From,
    string To,
    int TotalCount,
    int ReturnedCount,
...
public sealed record FaultTraceResponseDto(
    string OccurredAt,
    int Gw,
    string From,
    string To,
    IReadOnlyList<FaultTracePointDto> Points);

public sealed record FaultTracePointDto(
    string Sj,
    decimal? Current,
    decimal? Pressure,
    decimal? Flow,
