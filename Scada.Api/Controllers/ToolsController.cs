...
{
    private const string VncHostPrefix = "192.168.88.";
    private const int FirstVncHostSuffix = 51;
    private const int LastVncHostSuffix = 114;

    [HttpPost("vnc/open")]
    public ActionResult<VncOpenResponse> OpenVnc([FromBody] VncOpenRequest request)
    {
        var host = request.Host?.Trim() ?? string.Empty;
        if (!IsAllowedVncHost(host))
        {
    ...
    private static bool IsAllowedVncHost(string host)
    {
        if (!host.StartsWith(VncHostPrefix, StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }

        var suffixText = host[VncHostPrefix.Length..];
        return int.TryParse(suffixText, out var suffix)
            && suffix >= FirstVncHostSuffix
            && suffix <= LastVncHostSuffix;
    }
}
