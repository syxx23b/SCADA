...
type ViewKey =
  | 'dashboard'
  | 'nativeFactoryReportTest'
  | 'nativeEnduranceReportTest'
  | 'nativeGasEngineFactoryReportTest'
  | 'nativeGasEngineEnduranceReportTest'
  | 'motorFaultReport'
  | 'gasFaultReport'
  | 'efficiency'
  | 'fault'
...
const RECIPE_SUBSCRIPTION_LEASE_SECONDS = 20
...
// 全局操作反馈条：message 变化后显示约 3 秒自动隐藏；message 为空时不渲染。
function StatusToast({ message }: { message: string }) {
  const [hidden, setHidden] = useState(!message)
  const [prevMessage, setPrevMessage] = useState(message)

  // message 变化时重新显示（React 官方“在渲染期间调整状态”模式，避免在 effect 内同步 setState）
  ...
  return (
    <div className="toast-line" role="status" aria-live="polite">{message}</div>
  )
}

const baseSidebarItems: SidebarItem[] = [
  { key: 'dashboard', label: '数据看板', icon: <DashboardSidebarIcon /> },
  { key: 'nativeFactoryReportTest', label: '电机泵-出厂记录', icon: <FactoryRecordSidebarIcon /> },
  { key: 'nativeEnduranceReportTest', label: '电机泵-耐久记录', icon: <FactoryRecordSidebarIcon /> },
  { key: 'motorFaultReport', label: '电机泵-故障追溯', icon: <FaultTrackingSidebarIcon /> },
  { key: 'nativeGasEngineFactoryReportTest', label: '汽油机-出厂记录', icon: <FactoryRecordSidebarIcon /> },
  { key: 'nativeGasEngineEnduranceReportTest', label: '汽油机-耐久记录', icon: <FactoryRecordSidebarIcon /> },
  { key: 'gasFaultReport', label: '汽油机-故障追溯', icon: <FaultTrackingSidebarIcon /> },
  ...
  { key: 'workOrderCreate', label: '工单创建', icon: <WorkOrderCreateSidebarIcon /> },
...
const configurableMenuItems: Array<{ key: ConfigurableMenuKey; title: string; group: string; description: string }> = [
  { key: 'nativeFactoryReportTest', title: '电机泵-出厂记录', group: '原生报表', description: 'Motor-FactoryRpt.' },
  { key: 'nativeEnduranceReportTest', title: '电机泵-耐久记录', group: '原生报表', description: 'Motor-EnduranceRpt.' },
  { key: 'nativeGasEngineFactoryReportTest', title: '汽油机-出厂记录', group: '原生报表', description: 'Engine-FactoryRpt.' },
  { key: 'nativeGasEngineEnduranceReportTest', title: '汽油机-耐久记录', group: '原生报表', description: 'Engine-EnduranceRpt.' },
  { key: 'motorFaultReport', title: '电机泵-故障追溯', group: '原生报表', description: 'Motor-FaultTraceRpt.' },
  { key: 'gasFaultReport', title: '汽油机-故障追溯', group: '原生报表', description: 'Engine-FaultTraceRpt.' },
  { key: 'recipeDj', title: '配方-电机泵', group: '工单管理', description: '电机泵配方创建与配方管理' },
  { key: 'recipeQyj', title: '配方-汽油机', group: '工单管理', description: '汽油机配方创建与配方管理' },
  { key: 'workOrderCreate', title: '工单创建', group: '工单管理', description: '基于电机泵配方创建与维护工单' },
]
const DEFAULT_VISIBLE_MENU_KEYS: ConfigurableMenuKey[] = configurableMenuItems.map((item) => item.key)
const CONFIGURABLE_MENU_KEY_SET = new Set<SidebarKey>(DEFAULT_VISIBLE_MENU_KEYS)
const REQUIRED_VISIBLE_MENU_KEYS: ConfigurableMenuKey[] = ['workOrderCreate', 'motorFaultReport', 'gasFaultReport']
...
const NATIVE_REPORT_LANGUAGE_STORAGE_KEY = 'scada-web.native-report-language'
...
type WorkOrderServiceKey = 'recipeDj' | 'recipeQyj' | 'workOrderCreate'
const REWORK_SERVICE_KEYS: ReworkServiceKey[] = ['rework', 'reworkConfig', 'reworkRecords']
const WORK_ORDER_SERVICE_KEYS: WorkOrderServiceKey[] = ['recipeDj', 'recipeQyj', 'workOrderCreate']
...
function normalizeVisibleMenuKeys(values?: string[] | null): ConfigurableMenuKey[] {
  if (!values?.length) return DEFAULT_VISIBLE_MENU_KEYS
  const allowed = new Set<ConfigurableMenuKey>(DEFAULT_VISIBLE_MENU_KEYS)
  const normalized = values.filter((value): value is ConfigurableMenuKey => allowed.has(value as ConfigurableMenuKey))
  return Array.from(new Set([...normalized, ...REQUIRED_VISIBLE_MENU_KEYS]))
}

function areVisibleMenuKeysEqual(left: ConfigurableMenuKey[], right: ConfigurableMenuKey[]) {
...
function readInitialNativeReportLanguage(): 'zh' | 'en' {
  try {
    return window.localStorage.getItem(NATIVE_REPORT_LANGUAGE_STORAGE_KEY) === 'en' ? 'en' : 'zh'
  } catch {
    return 'zh'
  }
}
...
function BrandAnimatedTitle() {
  return (
    <svg className="brand-animated-title" viewBox="0 0 460 84" preserveAspectRatio="xMinYMid meet" role="img" aria-label="清洗机测试系统">
      <text x="0" y="52" dominantBaseline="middle">清洗机测试系统</text>
    </svg>
  )
}
...
function App() {
  ...
  const [nativeReportLanguage, setNativeReportLanguage] = useState<'zh' | 'en'>(readInitialNativeReportLanguage)
  ...
  const [visibleMenuKeys, setVisibleMenuKeys] = useState<ConfigurableMenuKey[]>(DEFAULT_VISIBLE_MENU_KEYS)
  ...
  const loadSystemConfig = useCallback(async () => {
    try {
      ...
      setVisibleMenuKeys(normalizeVisibleMenuKeys(settings.visibleMenuKeys))
  ...
  const sidebarItems = useMemo(() => (
    isAuthenticated
      ? [
          ...baseSidebarItems.filter((item) => !CONFIGURABLE_MENU_KEY_SET.has(item.key) || visibleMenuKeys.includes(item.key as ConfigurableMenuKey)).map((item) => {
            if (item.key === 'nativeFactoryReportTest') return { ...item, label: nativeReportLanguage === 'en' ? 'Motor-FactoryRpt.' : '电机泵-出厂记录' }
            if (item.key === 'nativeEnduranceReportTest') return { ...item, label: nativeReportLanguage === 'en' ? 'Motor-EnduranceRpt.' : '电机泵-耐久记录' }
            if (item.key === 'nativeGasEngineFactoryReportTest') return { ...item, label: nativeReportLanguage === 'en' ? 'Engine-FactoryRpt.' : '汽油机-出厂记录' }
            if (item.key === 'nativeGasEngineEnduranceReportTest') return { ...item, label: nativeReportLanguage === 'en' ? 'Engine-EnduranceRpt.' : '汽油机-耐久记录' }
            if (item.key === 'motorFaultReport') return { ...item, label: nativeReportLanguage === 'en' ? 'Motor-FaultTraceRpt.' : '电机泵-故障追溯' }
            if (item.key === 'gasFaultReport') return { ...item, label: nativeReportLanguage === 'en' ? 'Engine-FaultTraceRpt.' : '汽油机-故障追溯' }
            return item
          }),
  ...
  ), [isAuthenticated, nativeReportLanguage, visibleMenuKeys])
  const systemSettings = useMemo<SystemSettings>(() => ({
    stationCount,
    pressureUnit,
    flowUnit,
    visibleMenuKeys,
  }), [flowUnit, pressureUnit, stationCount, visibleMenuKeys])
  ...
  async function openVnc(faceplateIndex: number) {
    const host = `192.168.88.${50 + faceplateIndex}`
    try {
    ...
    saveSystemSettingsTimerRef.current = window.setTimeout(() => {
      void updateSystemSettings(systemSettings)
        .then((settings) => {
          ...
          const nextVisibleMenuKeys = normalizeVisibleMenuKeys(settings.visibleMenuKeys)
          setVisibleMenuKeys((current) => areVisibleMenuKeysEqual(current, nextVisibleMenuKeys) ? current : nextVisibleMenuKeys)
        })
        .catch((error) => {
  ...
  }, [flowUnit, pressureUnit, stationCount, systemSettings, systemSettingsLoaded, visibleMenuKeys])
  useEffect(() => {
    if (CONFIGURABLE_MENU_KEY_SET.has(view) && !visibleMenuKeys.includes(view as ConfigurableMenuKey)) {
      setView('dashboard')
    }
  }, [view, visibleMenuKeys])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    touchLease()
    const timer = window.setInterval(touchLease, 10000)
    return () => window.clearInterval(timer)
  }, [view])
  ...
  async function handleWrite(tagId: string, value?: string, options?: WriteOptions) {
    ...
    try {
      ...
      await touchRecipeSubscriptionLease('recipe-write', RECIPE_SUBSCRIPTION_LEASE_SECONDS)
  ...
  function handleConfigurableMenuToggle(key: ConfigurableMenuKey, checked: boolean) {
    setVisibleMenuKeys((current) => {
      if (checked) {
  ...
  function renderSidebarButtons(mode: 'default' | 'runtime') {
    ...
    const firstVisibleReportKey = sidebarItems.find((item) => reportServiceKeys.includes(item.key))?.key
    const lastVisibleReportKey = [...sidebarItems].reverse().find((item) => reportServiceKeys.includes(item.key))?.key
    const lastVisibleWorkOrderKey = [...sidebarItems].reverse().find((item) => WORK_ORDER_SERVICE_KEYS.includes(item.key as WorkOrderServiceKey))?.key
    ...
    const rendered = sidebarItems.flatMap((item) => {
      ...
      const shouldRenderReportGroupTail = item.key === lastVisibleReportKey
      const shouldRenderWorkOrderServiceTitle = item.key === firstVisibleWorkOrderKey
      const shouldRenderWorkOrderGroupTail = item.key === lastVisibleWorkOrderKey
      ...
      if (shouldRenderWorkOrderServiceTitle) {
        prefix.push(
          <div key={`work-order-service-divider-${mode}`} className="sidebar-divider" aria-hidden="true" />,
          <div key={`work-order-service-title-${mode}`} className="sidebar-group-title">
            工单管理
          </div>,
        )
      }
      const suffix = [
        ...
        ...(shouldRenderWorkOrderGroupTail ? [<div key={`work-order-group-tail-${mode}`} className="sidebar-divider" aria-hidden="true" />] : []),
          ...
          <div className="dashboard-center">
            ...
            <article key={`faceplate-${dashboardData.faceplateIndex}`} className="dashboard-board">
              <header className={`dashboard-board-head ${dashboardData.boardHeadClass}`}>
                ...
                >
                  <img src="/realvnc-logo.png" alt="" aria-hidden="true" />
                </button>
              </header>
  ...
  const workOrderCreatePage = (
    <>
      <WorkOrderManagement onStatus={showStatus} />
      <StatusToast message={statusMessage} />
    </>
  )
  ...
  const reportConfigPage = (
    <section className="page-shell report-page-shell report-config-shell">
      <section className="report-config-topbar">
        ...
        <article className="report-config-station-card report-config-list-card report-config-menu-card">
          <div className="panel-head">
            <div>
              <div className="panel-title">菜单可见性</div>
            </div>
          </div>
          <div className="report-config-list">
            {configurableMenuItems.map((item) => {
              const isVisible = visibleMenuKeys.includes(item.key)
              return (
                <div key={item.key} className="report-config-item">
                  <div className="report-config-info">
                    <strong>{item.title}</strong>
                    <span>{item.group}</span>
                  </div>
                  <div className="report-config-url">
                    <span className="report-config-url-text">{item.description}</span>
                  </div>
                  <label className={`report-config-toggle${isVisible ? ' is-visible' : ''}`}>
                    <input
                      type="checkbox"
                      checked={isVisible}
                      onChange={(event) => handleConfigurableMenuToggle(item.key, event.target.checked)}
                    />
                    {isVisible ? '显示' : '隐藏'}
                  </label>
                </div>
  ...
  if (view === 'nativeFactoryReportTest') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace"><NativeFactoryReport onStatus={showStatus} onLanguageChange={setNativeReportLanguage} /><StatusToast message={statusMessage} /></main></div>
  if (view === 'nativeEnduranceReportTest') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace"><NativeFactoryReport variant="endurance" onStatus={showStatus} onLanguageChange={setNativeReportLanguage} /><StatusToast message={statusMessage} /></main></div>
  if (view === 'nativeGasEngineFactoryReportTest') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace"><NativeFactoryReport variant="gasFactory" onStatus={showStatus} onLanguageChange={setNativeReportLanguage} /><StatusToast message={statusMessage} /></main></div>
  if (view === 'nativeGasEngineEnduranceReportTest') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace"><NativeFactoryReport variant="gasEndurance" onStatus={showStatus} onLanguageChange={setNativeReportLanguage} /><StatusToast message={statusMessage} /></main></div>
  if (view === 'motorFaultReport') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace"><MotorFaultReport onStatus={showStatus} onLanguageChange={setNativeReportLanguage} /><StatusToast message={statusMessage} /></main></div>
  if (view === 'gasFaultReport') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace"><MotorFaultReport variant="gas" onStatus={showStatus} onLanguageChange={setNativeReportLanguage} /><StatusToast message={statusMessage} /></main></div>
  ...
  if (view === 'workOrderCreate') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace">{workOrderCreatePage}</main></div>
