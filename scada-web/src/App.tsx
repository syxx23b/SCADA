...
const REQUIRED_VISIBLE_MENU_KEYS: ConfigurableMenuKey[] = ['workOrderCreate']
...
function getInitialView(): ViewKey {
  ...
  return value === 'dashboard' || value === 'nativeFactoryReportTest' || value === 'nativeEnduranceReportTest' || value === 'nativeGasEngineFactoryReportTest' || value === 'nativeGasEngineEnduranceReportTest' || value === 'efficiency' || value === 'fault' || value === 'production' || value === 'workOrderCreate' || value === 'rework' || value === 'reworkConfig' || value === 'reworkRecords' || value === 'runtime' || value === 'tags' || value === 'recipeDj' || value === 'recipeQyj' || value === 'reportConfig' || value === 'uploadInsertAudits' || value === 'help' || value === 'login' ? value : 'dashboard'
}
...
function App() {
  ...
  function renderSidebarButtons(mode: 'default' | 'runtime') {
    ...
    const reportServiceKeys: SidebarKey[] = ['nativeFactoryReportTest', 'nativeEnduranceReportTest', 'nativeGasEngineFactoryReportTest', 'nativeGasEngineEnduranceReportTest']
  ...
  if (view === 'nativeFactoryReportTest') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace"><NativeFactoryReport onStatus={showStatus} onLanguageChange={setNativeReportLanguage} /><div className="toast-line">{statusMessage}</div></main></div>
  if (view === 'nativeEnduranceReportTest') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace"><NativeFactoryReport variant="endurance" onStatus={showStatus} onLanguageChange={setNativeReportLanguage} /><div className="toast-line">{statusMessage}</div></main></div>
  if (view === 'nativeGasEngineFactoryReportTest') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace"><NativeFactoryReport variant="gasFactory" onStatus={showStatus} onLanguageChange={setNativeReportLanguage} /><div className="toast-line">{statusMessage}</div></main></div>
  if (view === 'nativeGasEngineEnduranceReportTest') return <div className={`app-shell${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>{sidebarShell}<main className="workspace"><NativeFactoryReport variant="gasEndurance" onStatus={showStatus} onLanguageChange={setNativeReportLanguage} /><div className="toast-line">{statusMessage}</div></main></div>
