...
export function ReworkManagement({
  ...
  const historyTimeline = useMemo(() => {
    ...
    for (const item of history?.repairItems ?? []) {
      ...
      events.push({ time: item.confirmedAt, type: 'repair', title: item.repairMeasure || '-', detail: '返修确认' })
    }
      ...
      <article className="production-card rework-query-card">
        <form className="rework-query-form" onSubmit={handleSubmit}>
          <label className="rework-query-label">
            <span>输入查询条码</span>
            <input className="rework-query-input" value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="请输入用于匹配 Error.tm 的条码" />
          </label>
          <button type="submit" className="primary-action rework-query-button" disabled={loading}>{loading ? '查询中...' : '查询'}</button>
        </form>
        ...
        {searched && result?.found ? (
          <div className="rework-history-timeline">
            <table className="runtime-table">
              <thead><tr><th>时间</th><th>类型</th><th>内容</th><th>备注</th></tr></thead>
              <tbody>
        ...
        <article className="production-table-card rework-result-card rework-repair-panel">
          ...
          <div className="rework-result-mindmap">
            ...
            <div className="rework-confirm-actions">
              {keyword.trim() ? (
                <button type="button" className="primary-action rework-confirm-button" disabled={confirming || (result.repairMeasures?.length ?? 0) === 0 || !selectedMeasure} onClick={() => void handleConfirmRepair()}>
