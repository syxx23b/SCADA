    ...
    <div className="recipe-container">
      ...
      {(() => {
        ...
        const matchesRecipeIndex = (normalizedPath: string, expectedIndex: number) => {
          ...
          const djRecipeMatch = normalizedPath.match(/^recipe_db\.djrecipe\[(\d+)\]\./i)
          if (djRecipeMatch) return Number(djRecipeMatch[1]) === expectedIndex

          const djRecipeUnderscoreMatch = normalizedPath.match(/^recipe_db_djrecipe(?:\[(\d+)\])?_/i)
          if (djRecipeUnderscoreMatch) return Number(djRecipeUnderscoreMatch[1] ?? `${expectedIndex}`) === expectedIndex
