...
export function ReworkConfig({ onStatus }: { onStatus: (message: string) => void }) {
  ...
  const loadEntries = async (err: number, silent = false) => {
    if (!silent) setLoading(true)
    try {
  ...
  const handleCreateSuggestion = async () => {
    ...
    if (!selectedErr) return onStatus('请先选择 ERR')
    setLoading(true)
    try {
  ...
  return (
    <section className="production-page rework-config-page">
      <header className="production-header">
        ...
        <div className="production-subtitle">建议按 ERR 独立管理，维修措施全局管理并跨 ERR 复用</div>
      </header>

      <div className="rework-config-shell">
        <aside className="production-card rework-config-panel">
          <div className="rework-config-panel-title">ERR 列表</div>
          <input
            className="rework-query-input"
            placeholder="搜索 ERR / 故障描述"
            value={errFilter}
        ...
        <main className="rework-config-main">
          ...
          <div className="rework-config-top">
            <section className="production-card rework-config-card">
              <div className="rework-config-panel-title">返修建议（当前 ERR）</div>
              <div className="rework-config-row">
                <input
                  className="rework-query-input"
                  placeholder="输入返修建议"
            ...
            <section className="production-card rework-config-card">
              <div className="rework-config-panel-title">当前 ERR 匹配关系</div>
              <div className="rework-config-panel-title rework-config-subtitle">建立当前 ERR 的措施匹配</div>
              <div className="rework-config-row">
          ...
          <section className="production-card rework-mindmap-card">
            <div className="rework-config-panel-title">思维导图（左建议 / 右措施）</div>
            {!selectedErrNode ? (
              <div className="production-empty-inline">请选择左侧 ERR</div>
            ) : (
